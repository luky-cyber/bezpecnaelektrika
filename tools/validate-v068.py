...
# v0.6.8 final production artifacts and release identity.
for rel in [
...
try:
    ...
    expected = {
        ...
        'channel': 'production',
        'date': '2026-09-18',
        'fingerprint': 'be-v0.6.8-production',
...
try:
    ...
    for k in ['businessLegalSetup', 'serviceScope', 'serviceArea', 'pricingModel', 'commercialContactPath', 'capacity', 'insuranceApplicability']:
        if required.get(k) != 'blocked':
            errors.append(f'Commercial decision must remain blocked in final v0.6.8: {k}')
except Exception as e:
...
if '# Bezpečná elektrika v0.6.8' not in readme or 'production release v0.6.8, promoted from accepted RC16' not in readme:
    errors.append('Final v0.6.8 README release identity is stale')
...
if '## v0.6.8 production promotion (2026-09-18)' not in release_notes:
    errors.append('Final v0.6.8 production promotion note missing')
...
if errors:
    ...
    for e in sorted(set(errors)):
        print(' -', e)
    raise SystemExit(1)
print('V0.6.8 CHECK OK · final production release + retained RC16 search revalidation, credential cold-start fit, navigation, design, a11y, safety and commercial guardrails')
